...
输出多维，加ANN
测试集工况包含在训练集内
timestep=24，hidden_unit=24，loss用reduce_mean，学习率0.0001
有正则项，只对最后一个cell的输出计算loss
隐状态神经元数5、ANN激活函数tanh
'''
...
sen = np.ones((96*11,24,4)) * 0.5
i = 0
for j in range(24):
    for k in range(4):
        sen[11*i+1,j,k] = sen[11*i+1,j,k] - sen[11*i+1,j,k]*0.1
        sen[11*i+2,j,k] = sen[11*i+2,j,k] + sen[11*i+2,j,k]*0.1
        sen[11*i+3,j,k] = sen[11*i+3,j,k] - sen[11*i+3,j,k]*0.2
        sen[11*i+4,j,k] = sen[11*i+4,j,k] + sen[11*i+4,j,k]*0.2
        sen[11*i+5,j,k] = sen[11*i+5,j,k] - sen[11*i+5,j,k]*0.3
        sen[11*i+6,j,k] = sen[11*i+6,j,k] + sen[11*i+6,j,k]*0.3
        sen[11*i+7,j,k] = sen[11*i+7,j,k] - sen[11*i+7,j,k]*0.4
        sen[11*i+8,j,k] = sen[10*i+8,j,k] + sen[11*i+8,j,k]*0.4
        sen[11*i+9,j,k] = sen[11*i+9,j,k] - sen[11*i+9,j,k]*0.5
        sen[11*i+10,j,k] = sen[11*i+10,j,k] + sen[11*i+10,j,k]*0.5
        i = i + 1
...
def train_lstm(batch_size, HIDDEN_UNIT, activation, gpu_options, Max_ITER):
    ...
    with tf.Session() as sess:
        ...
        for step in range(1, Max_ITER+1):
            ...
            if len(valid_loss) > 1:
                if ((valid_loss[-1]-valid_loss[-2]) > 0) or ((valid_loss[-2]-valid_loss[-1]) / valid_loss[-2] < 0.001):
...
def prediction(MODEL_SAVE_PATH_best, FIG_NAME_best, HIDDEN_UNIT_best, activation, gpu_options):
    ...
    with tf.Session() as sess:
        ...
        if ckpt and ckpt.model_checkpoint_path:
            ...
            #敏感性分析
            predict_sen = sess.run(pred, feed_dict={X:sen})
            predictions_sen = np.array(predict_sen)*(CL_max - CL_min) + CL_min
            #predictions_train = np.array(predict_train)*std + mean
            
#            sen_value = np.zeros(96)
#            for i in range(96):
#                sen_value[i] = np.mean([abs(predictions_sen[3*i]-predictions_sen[3*i+1])/predictions_sen[3*i]/0.5,
#                                        abs(predictions_sen[3*i]-predictions_sen[3*i+2])/predictions_sen[3*i]/0.5])

            sen_value = np.zeros(96)
            for i in range(96):
#                sen_value[i] = np.std([abs((predictions_sen[11*i]-predictions_sen[11*i+1])/predictions_sen[11*i]/0.1),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+2])/predictions_sen[11*i]/0.1),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+3])/predictions_sen[11*i]/0.2),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+4])/predictions_sen[11*i]/0.2),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+5])/predictions_sen[11*i]/0.3),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+6])/predictions_sen[11*i]/0.3),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+7])/predictions_sen[11*i]/0.4),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+8])/predictions_sen[11*i]/0.4),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+9])/predictions_sen[11*i]/0.5),
#                                        abs((predictions_sen[11*i]-predictions_sen[11*i+10])/predictions_sen[11*i]/0.5)])
                
                sen_value[i] = np.mean([abs((predictions_sen[11*i]-predictions_sen[11*i+1])/predictions_sen[11*i]/0.1),
                                        abs((predictions_sen[11*i]-predictions_sen[11*i+2])/predictions_sen[11*i]/0.1),
                                        abs((predictions_sen[11*i]-predictions_sen[11*i+3])/predictions_sen[11*i]/0.2),
                                        abs((predictions_sen[11*i]-predictions_sen[11*i+4])/predictions_sen[11*i]/0.2),
                                        abs((predictions_sen[11*i]-predictions_sen[11*i+5])/predictions_sen[11*i]/0.3),
...
#            plt.show()
#            
#            workbook = xlsxwriter.Workbook('./Prediction and error of train.xlsx')
...
#            worksheet = workbook.add_worksheet('Prediction error')
#            worksheet.write(0, 0, 'actual value')       
...
with tf.variable_scope('train_' + str(HIDDEN_UNIT_best) + '_' + ACTIVATION_best, reuse=True):
    sen_value = prediction(MODEL_SAVE_PATH_best, FIG_NAME_best, HIDDEN_UNIT_best, ACTIVATION_best, gpu_options)